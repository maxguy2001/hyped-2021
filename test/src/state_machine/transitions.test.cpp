...
struct TransitionFunctionality : public ::testing::Test {
  ...
  const std::string entered_braking_zone_error    = "Should handle entered braking zone."
...
TEST_F(TransitionFunctionality, handlesEnteredBrakingZone) 
{
  Navigation nav_data;

  bool entered_braking_zone;

  entered_braking_zone = checkEnteredBrakingZone(log, nav_data);

  ASSERT_EQ(entered_braking_zone, true) << entered_braking_zone_error;

}

// Only for testing purposes. The actual test code should be the one above.
TEST_F(TransitionFunctionality, handlesEnteredBrakingZoneTestValue) 
{
  bool entered_braking_zone;

  entered_braking_zone = hyped::state_machine::checkEnteredBrakingZoneTestValue(log, 500);
